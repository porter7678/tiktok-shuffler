...
export default function PlayerView({ currentVideo, onShuffle, onTrueShuffle, onNext, onPrev, hasNext, hasPrev, onHistoryBack, onHistoryForward, canBack, canForward, showTrueShuffle, onToggleLike, onToggleFavorite, onShowArtist, artistCount = 0, paused = false }) {
    ...
    <div className="player-view">
          ...
          <>
            ...
            {video.uploader && (
              <>
                ...
                <button className="artist-videos-btn" onClick={onShowArtist}>
                  See all {artistCount} video{artistCount === 1 ? '' : 's'} by this artist
                </button>
              </>
            )}
            {video.description && (
      ...
      <div className="video-wrapper">
        {!video
          ...
          : <VideoPlayer key={video.id} src={video.local_video_url} poster={video.local_thumbnail_url} playbackRate={playbackRate} videoId={video.id} onLoudness={setAudioInfo} replayToken={replayToken} paused={paused} />}
      </div>
      ...
      <aside className="side-controls">
        ...
        {showTrueShuffle && (
          <button className="true-shuffle-btn" onClick={onTrueShuffle} disabled={!video}>
            True Shuffle
          </button>
        )}
        ...
        <div className="history-row">
          <span className="history-label">Shuffle history</span>
          <div className="history-btns">
            <button className="nav-btn" onClick={onHistoryBack} disabled={!canBack} aria-label="Back in shuffle history">‹</button>
            <button className="nav-btn" onClick={onHistoryForward} disabled={!canForward} aria-label="Forward in shuffle history">›</button>
          </div>
        </div>
